...
for article in data:
    ...
    sentiment = article.get(
        "sentiment", ""
    ).lower()  # Ensure the sentiment is in lowercase

    # Parse the date and hour
    date_hour = datetime.strptime(published_at, "%Y-%m-%dT%H:%M:%S%z").replace(
    ...
    if date_hour not in sentiment_counts_by_datetime:
        sentiment_counts_by_datetime[date_hour] = {
            "positive": 0,
            "negative": 0,
            "neutral": 0,
        }
...
positive_counts = [
    sentiment_counts_by_datetime[dt]["positive"] for dt in sorted_datetimes
]
negative_counts = [
    sentiment_counts_by_datetime[dt]["negative"] for dt in sorted_datetimes
]
neutral_counts = [
    sentiment_counts_by_datetime[dt]["neutral"] for dt in sorted_datetimes
]
